...
grep "" server.log | cut -d " " -f 4- | sort | uniq
...
grep 'GET\|POST' server.log | cut -d " " -f 6-7 | sort | uniq -c | sort -rn
...
grep 'session created for user=' server.log | cut -d " " -f 8 | sort | uniq -c | sort -rn
